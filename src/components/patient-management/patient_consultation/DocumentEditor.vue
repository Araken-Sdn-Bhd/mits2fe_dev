  ...
  <div class="editor" ref="editor">
    ...
    >
      ...
      <div
        ...
        :style="page_style(page_idx, page.template ? false : true)"
        :contenteditable="editable && !page.template ? true : false"
      >
        <component
          v-if="page.template"
          :is="page.template"
          v-bind.sync="page.props"
        />
      </div>
    </div>
  ...
  methods: {
    ...
    async reset_content () {
      ...
      this.pages = this.content.length
        ? this.content.map((content, content_idx) => ({
          uuid: this.new_uuid(),
          content_idx,
      ...
      for (const page of this.pages) {
        ...
        if (!this.content[page.content_idx]) { page_elt.innerHTML = '<div><br></div>' } else if (typeof this.content[page.content_idx] === 'string') {
          page_elt.innerHTML =
      ...
      for (let page_idx = 0; page_idx < this.pages.length; page_idx++) {
        ...
        if (
          ...
          (prev_page_modified_flag ||
            page_elt.innerHTML !== page.prev_innerHTML ||
            (next_page_elt &&
              !next_page.template &&
          ...
          if (
            page_elt.clientHeight <= this.pages_height &&
            next_page &&
